...
1. 下载：粘贴分享文本/链接 → 自动解析出抖音/B站等视频链接；抖音用浏览器抓流（无需 Cookie），B站/YouTube 等走 yt-dlp
2. 字幕：选择本地视频 → whisper.cpp 生成 SRT；默认同时烧录字幕到视频画面，输出 `*.subbed.mp4`；首次使用时若无模型会自动提示下载
3. 去重：选择本地视频 → 按强度一键生成二创变体（镜像/缩放裁切/色彩/噪点/模糊/色调/随机参数）
4. 配音：默认隐藏。需要时设置环境变量 `VIDEO_BRIDGE_SHOW_DUB=1` 重新显示；支持输入文字或 SRT，调用 Audio8 TTS 0.1B 本地配音；选择视频+SRT 时按字幕时间轴逐句同步配音
